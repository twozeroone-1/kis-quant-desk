...
실전 주문은 날짜/슬롯별 1회성 승인 파일 또는 `KIS_PROD_AUTO_CONFIRM=I_UNDERSTAND_REAL_ORDERS`가 있을 때만 제출됩니다. LLM 모드는 주문 승인 게이트가 아니며, `live-prod`는 하위호환 입력으로만 남아 리포트에 deprecation warning을 남깁니다.
...
| prod LLM | 기본 `off`. `shadow`/`live-prod`는 리포트용 판단만 수행하며 주문 제출 여부를 바꾸지 않습니다. |
...
Strategy Builder 공통 KIS REST wrapper는 `EGW00201` 응답을 감지하면 `1s, 2s, 4s, 8s, 16s, 32s, 최대 60s` 지수 backoff와 jitter를 적용해 재시도합니다. 자동화 후보군/랭킹은 실패 시 fallback 후보군으로 계속 진행하며, 보호주문 매도 실패는 보호주문 상태의 `next_retry_at`, `retry_count`, `last_error_code`, `unsupported_paths`에 기록됩니다.
