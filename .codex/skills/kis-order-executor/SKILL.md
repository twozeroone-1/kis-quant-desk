...
- 주문 요청은 항상 `builder_state`, `.kis.yaml`, 또는 명시된 프리셋/파라미터 기준으로 전략 조건을 먼저 확인한다.
- 사용자가 종목을 지정하지 않았으면 대표 종목 몇 개로 바로 주문하지 않는다. 후보군을 선별하고 후보군 전체 신호를 먼저 실행한다.
- 신호 결과는 `BUY` / `SELL` / `HOLD`로 분리해 표로 보여준 뒤 주문 후보만 추린다.
- `SELL` 신호는 보유 수량이 있을 때만 매도 대상으로 처리한다.
- **실전(prod) 주문**: 종목명·수량·예상금액 명시 후 반드시 사용자 확인 요청
- **주문 전**: 현재 모드(vps/prod) 반드시 고지
- **신호 강도 < 0.5**: 자동으로 주문 건너뜀
- 신규 매수 전 익절가·손절가·자동 설정 가능 여부를 계산한다. OCO/조건부 주문이 앱 레벨 감시라면 서버 중단 시 한계를 명시한다.
- 신규 매수 후 보유/미체결 상태를 재조회해 접수 여부와 보호주문 상태를 확인한다.
...
사용자가 종목을 지정하지 않은 경우:
- 당일 후보군을 섹터·유동성·거래 가능 여부 기준으로 선별한다.
- 후보군 전체에 전략을 실행하고, 신호표 확인 전에는 주문하지 않는다.
- "지금 들어가도 돼?" 같은 질문은 직접 추천이 아니라 전략 기반 신호 확인으로 처리한다.
...
실시간 실행 전략 목록은 strategy_builder의 `/api/strategies`가 source of truth다.
백테스터 MCP 프리셋 목록과 다를 수 있으므로 실행 직전 API 결과를 기준으로 한다.
...
신호표는 아래처럼 주문 전 반드시 분리한다.

| 구분 | 처리 |
|------|------|
| BUY | 강도 0.5 이상, 매수 가능금액·리스크 조건 충족 시 주문 후보 |
| SELL | 보유 수량이 있고 강도 0.5 이상일 때만 주문 후보 |
| HOLD | 주문 제외. 이유만 보고 |
...
주문 전 체크:
- 현재가·매수가능금액·보유수량·미체결 주문을 조회한다.
- BUY는 익절/손절 기준과 보호주문 설정 가능 여부를 함께 표시한다.
- 해외주식은 지정가만 허용되는 경우가 있으므로 시장별 주문 제약을 확인한다.
...
**모의(vps) 시** — 신호·리스크 조건이 충족되면 사용자 확인 없이 실행 가능하지만, 주문 전 요약은 표시한다.
...
주문 후에는 보유/미체결/보호주문 상태를 재조회하고, ExecutionLog에서 타임스탬프별 이벤트를 추적한다.
