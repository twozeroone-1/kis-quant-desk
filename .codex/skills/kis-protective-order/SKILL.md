...
- 모의 미국 보호매도는 KIS 브로커 예약으로 우회하지 않는다. 앱 큐가 일반 모의 지정가 주문을 제출하고 보유수량·미체결을 확인한다.
- 미국 지정가 오프셋은 손절/익절별로 설정하며, 도달 후 주문가는 현재가보다 설정 비율만큼 낮게 계산해 체결 가능성을 높인다.
- 주문번호 수신은 완료가 아니다. `submitted_unconfirmed`로 유지하고 보유수량이 사라진 뒤에만 `filled`로 처리한다.
- 미체결이 재가격 주기를 넘기면 기존 주문을 취소하고 최신 현재가 기준 지정가로 다시 제출한다.
- 부분체결 주문은 미체결 잔량만 취소하고, 남은 보유수량으로 재주문한다.
- 재가격할 때마다 `us_exit_reprice_step_pct`만큼 하향 폭을 확대하되 `us_exit_max_offset_pct`를 넘지 않는다.
- 감시 주기는 잔고·미체결을 환경/시장/거래소별로 한 번만 조회해 공유하며, 상태는 `health` 필드로 확인한다.
- 감시 지연, 반복 호출 제한, 장기 미체결은 `/review` 경고와 Telegram 환경 설정이 있을 때 알림으로 노출한다.
