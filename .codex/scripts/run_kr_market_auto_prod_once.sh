...
if [[ "${KIS_PROD_AUTO_CONFIRM:-}" != "I_UNDERSTAND_REAL_ORDERS" ]]; then
  echo "warning: KIS_PROD_AUTO_CONFIRM is not set; prod run will generate a report but submit no orders"
fi
