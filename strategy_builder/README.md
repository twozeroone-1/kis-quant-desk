...
│   /api/orders/protective     손익절 감시/실시간 상태       │
│   /api/auth/*                KIS API 인증                │
│   /api/market/*              시세/호가 조회              │
└────────────────────┬─────────────────────────────────────┘
                     │
...
| 전략 검토 (`/review`) | 보유 종목 손절/익절 감시, 실시간 체결가 표시, 조건 도달 시 매도 |
...
상단 네비게이션에서 **전략 검토**를 클릭하면 `/review` 페이지로 이동합니다. 이 화면은 새 전략을 만드는 곳이 아니라, 현재 계좌에 이미 보유 중인 종목에 **손절/익절 감시 조건**을 붙이는 운영 화면입니다.

### 화면 구성

| 영역 | 설명 |
|------|------|
| 한국 / 미국 탭 | 시장별 보유 종목을 분리해서 조회합니다. 한국은 국내 잔고 API, 미국은 해외주식 잔고 API를 사용합니다. |
| 감시 주기 | WebSocket 틱이 없을 때 사용하는 REST fallback 점검 주기입니다. 기본 15초, 설정 범위 5~300초입니다. |
| 새로고침 | 보유종목, 예수금, 보호주문 상태를 다시 조회합니다. |
| 지금 점검 | 자동 감시 주기와 관계없이 보호주문 감시 루프를 즉시 1회 실행합니다. |
| 실시간 상태 카드 | WebSocket 연결 상태와 현재 구독 중인 종목 수를 표시합니다. |
| 종목 카드 | 수량, 평균가, 현재가, 평가금액, 손익률, 감시 설정을 종목별로 표시합니다. |

### 종목별 설정

각 종목 카드에는 아래 입력값이 있습니다.

| 입력값 | 설명 |
|--------|------|
| 감시 사용 | 켜면 해당 종목의 보호주문 규칙이 활성화됩니다. 끄고 저장하면 활성 감시가 비활성화됩니다. |
| 감시 수량 | 조건 도달 시 매도할 수량입니다. 실제 보유 수량 이하로 입력해야 합니다. |
| 손절/수익보존 매도 | 좌측 블록입니다. 현재가가 도달가 이하가 되면 설정한 방식으로 매도합니다. |
| 익절 매도 | 우측 블록입니다. 현재가가 도달가 이상이 되면 설정한 방식으로 매도합니다. |
| 도달가 | 조건을 평가하는 기준 가격입니다. |
| 주문 | `시장가` 또는 `지정가`를 선택합니다. |
| 지정가 | 주문 방식이 지정가일 때 제출할 주문 가격입니다. |

### `지금 점검` 버튼

`지금 점검`은 저장된 보호주문 조건을 서버에서 즉시 한 번 평가하는 수동 버튼입니다.

일반적으로는 백엔드 보호주문 서비스가 자동으로 감시합니다. 다만 아래 상황에서는 `지금 점검`이 유용합니다.

- 방금 감시 조건을 저장했고 즉시 서버 상태를 확인하고 싶을 때
- WebSocket 연결이 막 시작되기 전 조건 충족 여부를 확인하고 싶을 때
- 장외 또는 네트워크 문제로 실시간 틱이 늦게 들어오는 상황에서 REST fallback을 바로 실행하고 싶을 때
- UI에 표시된 마지막 점검 시각과 오류 메시지를 갱신하고 싶을 때

동작 순서는 다음과 같습니다.

1. `POST /api/orders/protective/check` 호출
2. 서버가 저장된 활성 보호주문을 순회
3. 현재 보유 수량 확인
4. 실시간 틱 또는 REST 현재가 기준으로 조건 평가
5. 익절/손절 조건이 충족되면 설정된 시장가 또는 지정가 매도 주문 제출
6. 보호주문 상태와 마지막 점검 시각 갱신

### 실시간 WebSocket 감시

전략 검토는 KIS WebSocket 체결가 스트림을 사용합니다.

| 시장 | KIS TR | 설명 |
|------|--------|------|
| 한국 | `H0STCNT0` | 국내주식 실시간 체결가 |
| 미국 | `HDFSCNT0` | 해외주식 실시간지연체결가. 미국은 공식 샘플상 무료 0분 지연 시세로 설명됩니다. |

백엔드는 하나의 공용 실시간 시세 스트림 서비스를 띄우고, 감시 중인 종목과 현재 화면에 표시되는 보유 종목을 같은 연결에서 구독합니다.

동작 방식:

1. 앱 내부 KIS 인증이 완료되면 WebSocket 승인키를 발급합니다.
2. 감시 중인 종목 또는 화면에 보이는 보유 종목을 KIS WebSocket에 구독합니다.
3. 체결가 틱이 들어오면 화면의 현재가를 갱신합니다.
4. 같은 틱으로 보호주문 엔진이 손절/익절 조건을 즉시 평가합니다.
5. 최근 30초 안에 실시간 틱이 들어온 종목은 REST 현재가 조회를 건너뜁니다.
6. WebSocket 연결이 없거나 틱이 오래 들어오지 않은 종목은 감시 주기 설정값에 따라 REST fallback으로 점검합니다.

### 관련 API

| Method | Path | 설명 |
|--------|------|------|
| `GET` | `/api/orders/protective` | 보호주문 목록, 감시 설정, 실시간 스트림 상태 조회 |
| `POST` | `/api/orders/protective` | 보유 종목 손절/익절 감시 설정 저장 |
| `POST` | `/api/orders/protective/settings` | 감시 주기 설정 저장 |
| `POST` | `/api/orders/protective/check` | 감시 조건 즉시 1회 점검 |
| `WS` | `/api/orders/protective/prices/ws` | 전략 검토 화면용 실시간 체결가 스트림 |
| `GET` | `/api/overseas/holdings` | 미국 보유 종목 조회 |
| `GET` | `/api/overseas/balance` | 미국 계좌 평가금액/예수금 조회 |

### 운영 주의사항

- 이 기능은 KIS 서버에 OCO 주문을 맡기는 방식이 아니라 앱 레벨 감시입니다.
- 브라우저를 닫아도 백엔드 컨테이너가 살아 있으면 감시는 계속됩니다.
- 백엔드가 중단되면 자동 감시도 멈춥니다.
- 실전 주문 전에는 반드시 모의투자에서 저장, 점검, 주문 제출 흐름을 확인하세요.
- 해외주식 시장가 처리는 거래소/권한/주문 구분값에 따라 제약이 있으므로, 운영 초기에는 지정가 위주로 검증하는 것을 권장합니다.
- WebSocket 구독 종목 수는 KIS 제한을 고려해야 합니다. 현재 서비스는 40개 초과 구독을 방어합니다.
...
| `GET` | `/api/orders/protective` | 보호주문/전략 검토 감시 상태 조회 |
| `POST` | `/api/orders/protective` | 보유 종목 손절/익절 감시 설정 저장 |
| `POST` | `/api/orders/protective/settings` | 감시 주기 설정 저장 |
| `POST` | `/api/orders/protective/check` | 감시 조건 즉시 1회 점검 |
| `WS` | `/api/orders/protective/prices/ws` | 전략 검토 실시간 체결가 스트림 |
...
│       └── symbols.py          # 종목 검색
│   └── services/
│       ├── protective_orders.py      # 앱 레벨 손익절 보호주문
│       └── realtime_price_stream.py  # KIS WebSocket 체결가 스트림
│
